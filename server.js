...
async function processStage1(prompt) {
...
async function processStage3(stage2Response) {
...
app.post('/review/stage1', async (req, res) => {
    try {
        ...
        const response = await processStage1(prompt);
        res.json({ content: response, model: "GPT-4", status: "Initial Analysis" });
    } catch (error) {
...
app.post('/review/stage3', async (req, res) => {
    try {
        const { stage2Response } = req.body;
        if (!stage2Response) {
            return res.status(400).json({ error: 'Stage 2 response is required' });
        }
        const response = await processStage3(stage2Response);
        res.json({ content: response, model: "GPT-4", status: "Final Refinement" });
    } catch (error) {
